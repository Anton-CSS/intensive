...
const generateSubCatalog = () =>{
  ...
  const subCatalogHTML = document.createElement('div');
  subCatalogHTML.classList.add('subcatalog');
  
  const updateHTML = (header, list) =>{
    subCatalogHTML.textContent = '';
    let listHTML = '';
    list.forEach(item => {
      listHTML += `<li class="subcatalog-list__item">
    ...
    <a href="goods.html?subcat=${header}">${header}</a>
    </h3>
  ...
  `;
    subCatalogHTML.insertAdjacentHTML('afterbegin', subCatalog);
  }
  document.body.insertAdjacentElement('beforeend', subCatalogHTML);
